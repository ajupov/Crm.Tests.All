...
{
    public class OrderTypeChangesTests
    {
        private readonly ICreate _create;
        private readonly IDefaultRequestHeadersService _defaultRequestHeadersService;
        private readonly IOrderTypesClient _orderTypesClient;
        private readonly IOrderTypeChangesClient _typeChangesClient;
        ...
        public async Task WhenGetPagedList_ThenSuccess()
        {
            ...
            var request = new OrderTypeChangeGetPagedListRequest
            {
                TypeId = type.Id,
            ...
            Assert.NotNull(response.Changes.First().NewValueJson.FromJsonString<OrderType>());
            Assert.True(!response.Changes.Last().OldValueJson.IsEmpty());
            Assert.True(!response.Changes.Last().NewValueJson.IsEmpty());
            Assert.False(response.Changes.Last().OldValueJson.FromJsonString<OrderType>().IsDeleted);
            Assert.True(response.Changes.Last().NewValueJson.FromJsonString<OrderType>().IsDeleted);
            Assert.Equal(response.Changes.Last().NewValueJson.FromJsonString<OrderType>().Name, type.Name);
        }
    }
}
